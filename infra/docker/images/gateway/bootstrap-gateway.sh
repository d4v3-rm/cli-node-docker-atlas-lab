...
render_vars='${LAB_HTTPS_PORT} ${GITLAB_HTTPS_PORT} ${OPENWEBUI_HTTPS_PORT} ${OLLAMA_HTTPS_PORT} ${PENPOT_HTTPS_PORT} ${TRILIUM_HTTPS_PORT} ${NODE_DEV_HTTPS_PORT} ${PYTHON_DEV_HTTPS_PORT} ${BOOKSTACK_HTTPS_PORT} ${N8N_HTTPS_PORT} ${LAB_PUBLIC_HOST} ${LAB_GATEWAY_IP} ${LAB_LOCAL_URL} ${LAB_URL} ${ATLAS_AI_LLM_ENABLED} ${ATLAS_WORKBENCH_ENABLED} ${GITLAB_URL} ${GITLAB_EXTERNAL_URL} ${OPENWEBUI_URL} ${OLLAMA_URL} ${PENPOT_URL} ${TRILIUM_URL} ${NODE_DEV_URL} ${PYTHON_DEV_URL} ${BOOKSTACK_URL} ${N8N_URL} ${GITLAB_ROOT_USERNAME} ${GITLAB_ROOT_PASSWORD} ${GITLAB_ROOT_EMAIL} ${BOOKSTACK_ROOT_NAME} ${BOOKSTACK_ROOT_EMAIL} ${BOOKSTACK_ROOT_PASSWORD} ${OPENWEBUI_ROOT_NAME} ${OPENWEBUI_ROOT_EMAIL} ${OPENWEBUI_ROOT_PASSWORD} ${N8N_ROOT_FIRST_NAME} ${N8N_ROOT_LAST_NAME} ${N8N_ROOT_EMAIL} ${N8N_ROOT_PASSWORD} ${PENPOT_ROOT_NAME} ${PENPOT_ROOT_EMAIL} ${PENPOT_ROOT_PASSWORD} ${OLLAMA_GATEWAY_USER} ${OLLAMA_GATEWAY_PASSWORD} ${OLLAMA_GATEWAY_PASSWORD_HASH} ${POSTGRES_DEV_SUPERUSER} ${POSTGRES_DEV_PASSWORD} ${POSTGRES_DEV_DATABASE} ${POSTGRES_DEV_HOST_PORT} ${NODE_DEV_PASSWORD} ${PYTHON_DEV_PASSWORD}'
