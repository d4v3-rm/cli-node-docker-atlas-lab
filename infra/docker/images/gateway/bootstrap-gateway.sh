...
render_vars='${LAB_HTTPS_PORT} ${GITEA_HTTPS_PORT} ${OPENWEBUI_HTTPS_PORT} ${OLLAMA_HTTPS_PORT} ${NODE_DEV_HTTPS_PORT} ${PYTHON_DEV_HTTPS_PORT} ${LAB_PUBLIC_HOST} ${LAB_GATEWAY_IP} ${LAB_LOCAL_URL} ${LAB_URL} ${ATLAS_AI_LLM_ENABLED} ${ATLAS_WORKBENCH_ENABLED} ${GITEA_URL} ${OPENWEBUI_URL} ${OLLAMA_URL} ${NODE_DEV_URL} ${PYTHON_DEV_URL} ${GITEA_ROOT_USERNAME} ${GITEA_ROOT_PASSWORD} ${GITEA_ROOT_EMAIL} ${OPENWEBUI_ROOT_NAME} ${OPENWEBUI_ROOT_EMAIL} ${OPENWEBUI_ROOT_PASSWORD} ${OLLAMA_GATEWAY_USER} ${OLLAMA_GATEWAY_PASSWORD} ${OLLAMA_GATEWAY_PASSWORD_HASH} ${POSTGRES_DEV_SUPERUSER} ${POSTGRES_DEV_PASSWORD} ${POSTGRES_DEV_DATABASE} ${POSTGRES_DEV_HOST_PORT} ${NODE_DEV_PASSWORD} ${PYTHON_DEV_PASSWORD}'
