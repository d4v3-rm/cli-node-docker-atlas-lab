...
ATLAS_AI_AGENTS_ENABLED="${ATLAS_AI_AGENTS_ENABLED:-false}"
ATLAS_AI_IMAGE_ENABLED="${ATLAS_AI_IMAGE_ENABLED:-false}"
ATLAS_AI_VIDEO_ENABLED="${ATLAS_AI_VIDEO_ENABLED:-false}"
ATLAS_WORKBENCH_ENABLED="${ATLAS_WORKBENCH_ENABLED:-false}"
export ATLAS_AI_LLM_ENABLED ATLAS_AI_AGENTS_ENABLED ATLAS_AI_IMAGE_ENABLED ATLAS_AI_VIDEO_ENABLED ATLAS_WORKBENCH_ENABLED
...
COMFYUI_GATEWAY_USER
COMFYUI_GATEWAY_PASSWORD
COMFYUI_REPO
COMFYUI_REVISION
COMFYUI_LTX_MODEL_TITLE
COMFYUI_WAN_MODEL_TITLE
...
COMFYUI_GATEWAY_PASSWORD_HASH="$(caddy hash-password --plaintext "${COMFYUI_GATEWAY_PASSWORD}")"

export OLLAMA_GATEWAY_PASSWORD_HASH INVOKEAI_GATEWAY_PASSWORD_HASH COMFYUI_GATEWAY_PASSWORD_HASH

render_vars='${LAB_HTTPS_PORT} ${GITEA_HTTPS_PORT} ${N8N_HTTPS_PORT} ${OPENWEBUI_HTTPS_PORT} ${OLLAMA_HTTPS_PORT} ${INVOKEAI_HTTPS_PORT} ${COMFYUI_HTTPS_PORT} ${NODE_DEV_HTTPS_PORT} ${PYTHON_DEV_HTTPS_PORT} ${AI_DEV_HTTPS_PORT} ${CPP_DEV_HTTPS_PORT} ${LAB_PUBLIC_HOST} ${LAB_GATEWAY_IP} ${LAB_LOCAL_URL} ${LAB_URL} ${ATLAS_AI_LLM_ENABLED} ${ATLAS_AI_AGENTS_ENABLED} ${ATLAS_AI_IMAGE_ENABLED} ${ATLAS_AI_VIDEO_ENABLED} ${ATLAS_WORKBENCH_ENABLED} ${GITEA_URL} ${N8N_URL} ${OPENWEBUI_URL} ${OLLAMA_URL} ${INVOKEAI_URL} ${COMFYUI_URL} ${NODE_DEV_URL} ${PYTHON_DEV_URL} ${AI_DEV_URL} ${CPP_DEV_URL} ${GITEA_ROOT_USERNAME} ${GITEA_ROOT_PASSWORD} ${GITEA_ROOT_EMAIL} ${N8N_ROOT_FIRST_NAME} ${N8N_ROOT_LAST_NAME} ${N8N_ROOT_EMAIL} ${N8N_ROOT_PASSWORD} ${OPENWEBUI_ROOT_NAME} ${OPENWEBUI_ROOT_EMAIL} ${OPENWEBUI_ROOT_PASSWORD} ${OLLAMA_GATEWAY_USER} ${OLLAMA_GATEWAY_PASSWORD} ${OLLAMA_GATEWAY_PASSWORD_HASH} ${INVOKEAI_GATEWAY_USER} ${INVOKEAI_GATEWAY_PASSWORD} ${INVOKEAI_GATEWAY_PASSWORD_HASH} ${INVOKEAI_MODEL_REPO} ${INVOKEAI_MODEL_REVISION} ${INVOKEAI_MODEL_TITLE} ${COMFYUI_GATEWAY_USER} ${COMFYUI_GATEWAY_PASSWORD} ${COMFYUI_GATEWAY_PASSWORD_HASH} ${COMFYUI_REPO} ${COMFYUI_REVISION} ${COMFYUI_LTX_MODEL_TITLE} ${COMFYUI_WAN_MODEL_TITLE} ${POSTGRES_DEV_SUPERUSER} ${POSTGRES_DEV_PASSWORD} ${POSTGRES_DEV_DATABASE} ${POSTGRES_DEV_HOST_PORT} ${NODE_DEV_PASSWORD} ${PYTHON_DEV_PASSWORD} ${AI_DEV_PASSWORD} ${CPP_DEV_PASSWORD}'
